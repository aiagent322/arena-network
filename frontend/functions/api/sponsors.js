...
async function handleGet(request, db) {
    ...
    const result = await db.prepare(
        `SELECT s.*, a.name as arena_name FROM sponsors s LEFT JOIN arenas a ON s.arena_id = a.id ${whereClause} ORDER BY CASE s.sponsor_level WHEN 'platinum' THEN 1 WHEN 'gold' THEN 2 WHEN 'silver' THEN 3 WHEN 'bronze' THEN 4 ELSE 5 END, s.sponsor_name`
    ).bind(...binds).all();
