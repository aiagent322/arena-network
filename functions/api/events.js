...
const ALLOWED_FIELDS = ['arena_id','title','description','event_type','discipline','start_date','end_date',
    'start_time','end_time','entry_fee','added_money','promoter','website','contact_name','contact_phone',
    'contact_email','flyer_url','source_url','status','is_approved','is_scraped'];
...
async function handleGet(request, db) {
    ...
    const countResult = await db.prepare(`SELECT COUNT(*) as total FROM events e ${wc}`).bind(...binds).first();
    const total = countResult?.total || 0;

    const result = await db.prepare(
        `SELECT e.*, a.name as arena_name, a.city as arena_city, a.state as arena_state, a.slug as arena_slug
         FROM events e LEFT JOIN arenas a ON e.arena_id = a.id ${wc} ORDER BY e.start_date ASC LIMIT ? OFFSET ?`
    ).bind(...binds, limit, offset).all();

    return Response.json({ events: result.results || [], total, page, limit, pages: Math.ceil(total / limit) }, { headers: corsHeaders() });
}
...
async function handlePut(request, db) {
    ...
    for (const [key, value] of Object.entries(body)) {
        ...
        fields.push(`${key} = ?`);
        binds.push(key === 'is_approved' || key === 'is_scraped' ? (value ? 1 : 0) : value);
    }
    ...
    const result = await db.prepare(`UPDATE events SET ${fields.join(', ')} WHERE id = ? RETURNING *`).bind(...binds).first();
